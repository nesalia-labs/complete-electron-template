...
import { recentProjects, type RecentProject } from './recent-projects.js'
 ...
 */
export function listRecentProjects(db: AppDatabase, limit = 10): RecentProject[] {
  return db
    .select()
    .from(recentProjects)
 ...
 */
export function deleteRecentProject(db: AppDatabase, projectId: string): boolean {
  const result = db
