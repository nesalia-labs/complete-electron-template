 ...
 *
 * One trigger path dispatches a turn; everything else is ignored:
 *
 *   1. `issues` event with `action === "opened"` — a new issue lands.
 *
 * The re-triage trigger on `action === "labeled"` (a human adding
 * `status: triage` to summon the agent again) is **deferred to v1.5**.
 * The `eve@0.13.3` `GitHubIssueEvent` shape exposes only `action`,
 * `issueNumber`, and the issue sub-payload as `raw` — the label that
 * was just added lives on the top-level webhook envelope, which the
 * public `onIssue` signature does not surface. We don't reach into
 * the envelope by hand: that would mean relying on undocumented
 * payload shape, and the only safe time to add a re-triage trigger
 * is when eve exposes the added label explicitly. v1.5 will land
 * alongside the comment-driven `@triage-bot` surface (see
 * `docs/learnings/eve/issue-triage.md` § 11 / § 12).
 *
